...
getGOTerms <- function(i){
  ...
  t.gns <- tryCatch(expr = intraIDMapper(t.prots, species = "HOMSA", 
                                               srcIDType = "UNIPROT", destIDType = "SYMBOL"),
                          error = function(e) return(""))
  t.gns <- t.gns %>% base::unlist() %>% unique
  ...
  if(K > 0){
    for (i in 1:nTerms) {
      ...
      x.overlap.genes = intersect(GO.term.genes, t.gns)
...
clusterExport(cl=cl, varlist=c("all.GO.terms", "allTargetList", "getGOTerms", "V", "N", "ppi.net.all", "nTerms", "cutoff"), envir = .GlobalEnv)
result = parLapply(cl, seq_len(length(allTargetList)), fun = getGOTerms)
stopCluster(cl)
saveRDS(result, file = paste0("Data/result.", ontTag ,".JI.without.PPI.rds"))
...
# write.csv(mat, paste0("/srv/scratch/z3526914/DrugRepo/Data/GO_sim_JI_without_PPI_",ontTag,".csv"), quote = F, row.names = F)
# end_time <- Sys.time()
